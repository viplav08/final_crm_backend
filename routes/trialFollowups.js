...
router.patch("/:id", async (req, res) => {
  ...
  try {
    ...
    // … your existing "subscribed" and "unsubscribed" branches go here (unchanged) …

    // 4️⃣ Follow‑Up branch
    if (o === "follow up") {
      ...
      // Keep the trial record, just update its status/remarks
      await pool.query(
    ...
    // 5️⃣ Default: just update status + remarks
    await pool.query(
      "UPDATE trial_followups SET status = $1, remarks = $2 WHERE id = $3",
      [outcome, remarks, id]
    );
